...
import bottleneck  # noqa: F401 # isort: skip
...
def conform_dea_band_names(ds):
    ...
    # notify user
    logger.info("Conforming satellite bands")
    ...
    # notify user
    logger.info("> Satellite band names conformed successfully.\n")
...
def calc_vege_index(ds, index="ndvi", drop=True):
    ...
    # notify user
    logger.info("Generating vegetation index: {0}".format(index))
    ...
    if not set(ds.variables) & set(valid_bands):
        raise ValueError(
            "No valid spectral band names (e.g. red) exist in dataset. Aborting."
        )
    ...
    if drop:
        ...
        logger.info(
            "> Drop bands set to True. Dropping these bands: {0}".format(bands_to_drop)
        )
    ...
    if index is None:
        ...
        ds["veg_index"] = (2.5 * (ds.nir - ds.red)) / (
    ...
    # notify user
    logger.info("> Vegetation index calculated successfully.\n")

    return ds


def remove_outliers(ds, method="median", user_factor=2, z_pval=0.05) -> xr.Dataset:
    """
    ...
    # notify user
    logger.info(
        "Outlier removal method: {0} with a user factor of: {1}".format(
            method, user_factor
        )
    )
    ...
    if "time" not in list(ds.dims):
        raise ValueError(
            "Time dimension not in dataset. Please ensure dataset has a time dimension."
        )

    # check if dataset contains veg_index variable
    if "veg_index" not in list(ds.data_vars):
        raise ValueError(
            "Vegetation index (veg_index) not in dataset. Please generate veg_index first."
        )

    # check if dataset is 2D or above
    if len(ds["veg_index"].shape) == 1:
        raise Exception(
            "Remove outliers does not operate on 1D datasets. Ensure it has an x, y and time dimension."
        )

    # check if user factor provided
    if user_factor <= 0:
        raise TypeError(
            "User factor is less than 0. Please provide a value of 0 or above."
        )

    # check if pval provided if method is zscore
    if method == "zscore" and z_pval not in [0.1, 0.05, 0.01]:
        raise ValueError(
            "Zscore selected but invalid pvalue provided. Ensure it is either 0.1, 0.05 or 0.01."
        )
    ...
    if method in ["median", "zscore"]:
        ...
        if method == "median":
            ...
            win_size = int(win_size / len(ds.resample(time="1Y")))
            ...
            if z_pval == 0.01:
                ...
                raise ValueError(
                    "Zscore p-value not supported. Please use 0.1, 0.05 or 0.01."
                )
        ...
        lefts, rights = ds.shift(time=1).where(outlier_mask), ds.shift(time=-1).where(
            outlier_mask
        )
        ...
        outlier_mask = xr.where(
            (ds.where(outlier_mask) < (nbr_means - cutoffs))
            | (ds.where(outlier_mask) > (nbr_maxs + cutoffs)),
            True,
            False,
        )
...
def correct_last_datetime(ds, interval):
    ...
    # notify user
    logger.info("Correcting last datetime value to ensure adequate resampling output.")
    ...
    if interval == ["1M"]:
        logger.info("> No need to correct last datetime for monthly resampling.")
        return ds
    ...
    d, m, y = (
        int(old_dt["time.day"]),
        int(old_dt["time.month"]),
        int(old_dt["time.year"]),
    )
    if m == 12 and d < 31:
        # notify
        logger.info("> Changing day of last datetime value in dataset to the 31st.")
        ...
        # notify
        logger.info(
            "> Could not change day value as month is not December or day already the 31st."
        )

    # notify and return
    logger.info("> Corrected late datetime value successfully.")
    return ds
...
def remove_non_dominant_year(ds):
    ...
    if len(unique_year_values) == 0 or len(unique_year_values) > 2:
        ...
        logger.info("> More than one year detected in dataset. Removal non-dominant years.")

        # get max count index and associated year
        np.argmax(unique_year_counts)
    ...
    # notify and return
    logger.info("> Checked and removed non-dominant year (if needed) successfully.")
    return ds
...
def resample(ds, interval="1M", reducer="median"):
    ...
    # notify user
    logger.info("Resampling dataset interval: {0} via reducer: {1}".format(interval, reducer))
    ...
    if len(ds["veg_index"].shape) == 1:
        raise Exception(
            "Resample does not operate on 1D datasets. Ensure it has an x, y and time dimension."
        )
    ...
    if interval in ["1W", "2W", "1M"]:
        if reducer == "mean":
            ...
            raise ValueError(
                "Provided reducer not supported. Please use mean or median."
            )
    else:
        raise ValueError(
            "Provided resample interval not supported. Please use 1W, 2W or 1M."
        )
    ...
    # notify user
    logger.info("> Resample successful.\n")
...
def group(ds, group_by="month", reducer="median"):
    ...
    # notify user
    logger.info("Group dataset interval: {0} via reducer: {1}".format(group_by, reducer))
    ...
    # notify user
    logger.info("> Selecting year: {0} to re-label times after groupby.".format(year))
    ...
    if "month" in list(ds.dims):
        ...
        times = [
            datetime.strptime("{0} {1} {2}".format(year, int(dt), 0), "%Y %W %w")
            for dt in ds["time"]
        ]
        ds["time"] = [np.datetime64(dt) for dt in times]
...
def interpolate(ds, method="interpolate_na"):
    ...
    # notify user
    logger.info("Interpolating dataset using method: {0}.".format(method))
    ...
    if len(ds["veg_index"].shape) == 1:
        raise Exception(
            "Interpolate does not operate on 1D datasets. Ensure it has an x, y and time dimension."
        )
    ...
    if method in ["interpolate_na", "fast_fill"]:
        ...
        raise ValueError(
            "Provided method not supported. Please use interpolate_na or fast_fill"
        )
...
def smooth(ds, method="savitsky", window_length=3, polyorder=1, sigma=1):
    ...
    # notify user
    logger.info(
        "Smoothing method: {0} with window length: {1} and polyorder: {2}.".format(
            method, window_length, polyorder
        )
    )
    ...
    if window_length <= 0 or not isinstance(window_length, int):
        raise TypeError(
            "> Window_length is <= 0 and/or not an integer. Please provide a value of 0 or above."
        )

    # check if user factor provided
    if polyorder <= 0 or not isinstance(polyorder, int):
        raise TypeError(
            "> Polyorder is <= 0 and/or not an integer. Please provide a value of 0 or above."
        )

    # check if polyorder less than window_length
    if polyorder > window_length:
        raise TypeError(
            "> Polyorder is > than window_length. Must be less than window_length."
        )
    ...
    time_axis = list(ds.dims).index("time") if "time" in ds.dims else 0
    ...
    if method in ["savitsky", "symm_gaussian", "asymm_gaussian", "double_logistic"]:
        if method == "savitsky":
            ...
            def smoother(da, sigma):
                return da.apply(gaussian_filter, sigma=sigma, axis=time_axis)
...
def calc_num_seasons(ds):
    ...
    # notify user
    logger.info("Beginning calculation of number of seasons.")
    ...
    logger.info("> Calculating number of seasons.")
...
def create_ds_template(da):
    ...
    # notify user
    logger.info("Creating a dateset template to hold various phenometrics.")
    ...
    if "time" not in list(da.dims):
        raise ValueError(
            "Time dimension not in data array. Please ensure dataset has a time dimension."
        )

    # check if time dimension is in dataset
    if int(da["time"].count()) == 0:
        raise ValueError(
            "Time dimension has no time slices. Please ensure data array contains data."
        )
    ...
    # notify user
    logger.info("> Created blank dateset template successfully.\n")
...
#     # notify user
#     logger.info('Beginning addition of CRS metadata.')
#     try:
#         # notify user
#         logger.info('> Adding CRS metadata.')
...
#     except:
#         # notify user
#         logger.info('> Could not add CRS metadata to data. Aborting.\n')
#         pass
...
def get_pos(da):
    ...
    # notify user
    logger.info("Beginning calculation of peak of season (pos) values and times.")

    # get pos values (max val in each pixel timeseries)
    logger.info("> Calculating peak of season (pos) values.")
    da_pos_values = da.max("time")

    # get pos times (day of year) at max val in each pixel timeseries)
    logger.info("> Calculating peak of season (pos) times.")
    i = da.argmax("time", skipna=True)
    da_pos_times = da["time"].isel(time=i, drop=True)

    # convert type
    da_pos_values = da_pos_values.astype("float32")
    da_pos_times = da_pos_times.astype("datetime64[ns]")
...
def get_mos(da, da_peak_times):
    ...
    logger.info("> Calculating middle of season (mos) values.")
    slope_l = da.where(da["time"] <= da_peak_times)
    slope_r = da.where(da["time"] >= da_peak_times)
...
def get_vos(da):
    """
    Takes an xarray DataArray containing veg_index values and calculates the vegetation
    value and time (datetime64) at valley of season (vos) for each timeseries per-pixel.
    The valley of season is the minimum value in the timeseries, per-pixel.

    Parameters
    ----------
    da: xarray DataArray
        A two-dimensional or multi-dimensional array containing veg_index
        and time values.
    ...
    da_vos_times : xarray DataArray
        An xarray DataArray type with an x and y dimension (no time). Each pixel is the
        datetime64 value detected at the valley of season (vos).
    """

    # notify user
    logger.info("Beginning calculation of valley of season (vos) values and times.")

    # get vos values (min val in each pixel timeseries)
    logger.info("> Calculating valley of season (vos) values.")
    da_vos_values = da.min("time")

    # get vos times (datetime64) at min val in each pixel timeseries
    logger.info("> Calculating valley of season (vos) times.")
    i = da.argmin("time", skipna=True)
    da_vos_times = da["time"].isel(time=i, drop=True)

    # convert type
    da_vos_values = da_vos_values.astype("float32")
    da_vos_times = da_vos_times.astype("datetime64[ns]")
...
def get_bse(da, da_peak_times):
    ...
    peak of season (pos) data array (datetime64), which can either be the max of the timeseries,
    or the middle of season (mos) values.
    ...
    da: xarray DataArray
        ...
        values.
    da_peak_times: xarray DataArray
        An xarray DataArray with an x and y dimension (no time). Each pixel must be
        the datetime64 value calculated at either at peak of season (pos) or middle
        of season (mos) prior.
    ...
    da_bse_values : xarray DataArray
        An xarray DataArray type with an x and y dimension (no time). Each pixel is the
        base (bse) veg_index value detected across the timeseries at each pixel.
    """

    # notify user
    logger.info("Beginning calculation of base (bse) values (times not possible).")

    # get base values (min on left and min on right around the provided peak datetimes)
    logger.info("> Calculating base (bse) values.")

    # split timeseries into left and right slopes via provided peak/middle datetime values
    slope_l = da.where(da["time"] <= da_peak_times).min("time")
    slope_r = da.where(da["time"] >= da_peak_times).min("time")

    # get per-pixel mean of both left and right slope min values
    da_bse_values = (slope_l + slope_r) / 2.0
...
def get_aos(da_peak_values, da_base_values):
    ...
    logger.info("> Calculating amplitude of season (aos) values.")
...
def get_sos(
    ...
    da_peak_times: xarray DataArray
        An xarray DataArray type with an x and y dimension (no time). Each pixel is the
        datetime64 value detected at either the peak (pos) or middle (mos) of season.
    da_base_values: xarray DataArray
        An xarray DataArray type with an x and y dimension (no time). Each pixel is the
        veg_index value detected at either the base (bse) or valley (vos) of season.
    da_aos_values : xarray DataArray
    ...
    method: str
        ...
        same as TIMESAT: seasonal_amplitude. The available options include:
          first_of_slope, median_of_slope, seasonal_amplitude, absolute_value,
          relative_value, stl_trend.
    factor: float
        A float value between 0 and 1 used for amplitude-based thresholds.
    thresh_sides: str
        'one_sided' or 'two_sided' for seasonal_amplitude method.
    abs_value: float
        Absolute value threshold for absolute_value method.

    Returns
    -------
    da_sos_values : xarray DataArray
        veg_index value at start of season.
    da_sos_times : xarray DataArray
        datetime64[ns] time at start of season.
    """

    logger.info("Beginning calculation of start of season (sos) values and times.")
    ...
    if thresh_sides not in ["one_sided", "two_sided"]:
        raise ValueError(
            "Provided thresh_sides value is not one_sided or two_sided. Aborting."
        )

    if method == "first_of_slope":

        logger.info("> Calculating start of season (sos) values via method: first_of_slope.")

        # left slope (datetime comparison)
        slope_l = da.where(da["time"] <= da_peak_times)
        ...
        logger.info("> Calculating start of season (sos) times via method: first_of_slope.")
        da_sos_times = slope_l_pos["time"].isel(time=i, drop=True)

    elif method == "median_of_slope":

        logger.info("> Calculating start of season (sos) values via method: median_of_slope.")
        ...
        logger.info("> Calculating start of season (sos) times via method: median_of_slope.")
        da_sos_times = slope_l_pos["time"].isel(time=i, drop=True)
        ...
        logger.info("> Calculating start of season (sos) values via method: absolute_value.")
        ...
        logger.info("> Calculating start of season (sos) times via method: absolute_value.")
        da_sos_times = slope_l_pos["time"].isel(time=i, drop=True)

    elif method == "relative_value":

        logger.info("> Calculating start of season (sos) values via method: relative_value.")
        ...
        relative_amplitude = da.quantile(dim="time", q=0.90) - da.quantile(
            dim="time", q=0.10
        )
        ...
        logger.info("> Calculating start of season (sos) times via method: relative_value.")
        logger.info("> Warning: this can take a long time.")

        da_sos_times = slope_l_pos["time"].isel(time=i, drop=True)

    elif method == "stl_trend":

        logger.info("> Calculating end of season (eos) values via method: stl_trend.")

        num_periods = len(da["time"])
        if num_periods % 2 == 0:
            num_periods = num_periods + 1
            logger.info("> Number of stl periods is even number, added 1 to make it odd.")
        ...
        logger.info(
            "> Performing seasonal decomposition via LOESS. Warning: this can take a long time."
        )
        ...
        logger.info("> Calculating start of season (sos) values via method: stl_trend.")
        ...
        stl_l = da_stl.where(da_stl["time"] <= da_peak_times)
        ...
        logger.info("> Calculating start of season (sos) times via method: stl_trend.")
        da_sos_times = slope_l_pos["time"].isel(time=i, drop=True)

    else:
        raise ValueError("Provided method not supported. Aborting.")

    # replace any all nan slices with nan
    da_sos_values = da_sos_values.where(~mask, np.nan)
    da_sos_times = da_sos_times.where(~mask, np.datetime64("NaT"))

    # convert type
    da_sos_values = da_sos_values.astype("float32")
    da_sos_times = da_sos_times.astype("datetime64[ns]")
...
def get_eos(
    ...
    """
    Same behaviour as original get_eos but comparisons and output times use datetime64[ns].
    """
    logger.info("Beginning calculation of end of season (eos) values and times.")
    ...
    if method == "first_of_slope":
        ...
        logger.info("> Calculating end of season (eos) values via method: first_of_slope.")

        # right slope (datetime comparison)
        slope_r = da.where(da["time"] >= da_peak_times)
        ...
        logger.info("> Calculating end of season (eos) times via method: first_of_slope.")
        da_eos_times = slope_r_neg["time"].isel(time=i, drop=True)

    elif method == "median_of_slope":

        logger.info("> Calculating end of season (eos) values via method: median_of_slope.")
        ...
        logger.info("> Calculating end of season (eos) times via method: median_of_slope.")
        da_eos_times = slope_r_neg["time"].isel(time=i, drop=True)
        ...
        logger.info("> Calculating end of season (eos) times via method: seasonal_amplitude.")
        da_eos_times = slope_r_neg["time"].isel(time=i, drop=True)

    elif method == "absolute_value":

        logger.info("> Calculating end of season (eos) values via method: absolute_value.")
        ...
        logger.info("> Calculating end of season (eos) times via method: absolute_value.")
        da_eos_times = slope_r_neg["time"].isel(time=i, drop=True)

    elif method == "relative_value":

        logger.info("> Calculating end of season (eos) values via method: relative_value.")
        ...
        logger.info("> Calculating end of season (eos) times via method: relative_value.")
        logger.info("> Warning: this can take a long time.")
        da_eos_times = slope_r_neg["time"].isel(time=i, drop=True)
        ...
        stl_r = da_stl.where(da_stl["time"] >= da_peak_times)
        ...
        logger.info("> Calculating end of season (eos) times via method: stl_trend.")
        da_eos_times = slope_r_neg["time"].isel(time=i, drop=True)
    ...
    da_eos_times = da_eos_times.where(~mask, np.datetime64("NaT"))

    # convert type
    da_eos_values = da_eos_values.astype("float32")
    da_eos_times = da_eos_times.astype("datetime64[ns]")
...
def get_los(da, da_sos_times, da_eos_times):
    """
    Takes two xarray DataArrays containing the start of season (sos) times and
    end of season (eos) times (datetime64) and calculates the length of season (los).
    This is calculated as eos day of year minus sos day of year per pixel, with the
    same wrap-around correction the original used.

    Returns da_los_values (int16 days).
    """
    ...
    # compute day-of-year from datetime inputs (works if inputs are datetime64)
    da_eos_doy = da_eos_times.dt.dayofyear
    da_sos_doy = da_sos_times.dt.dayofyear

    # get los values (eos day of year - sos day of year)
    logger.info("> Calculating length of season (los) values.")
    da_los_values = da_eos_doy - da_sos_doy

    # correct los if negative values exist (season crosses year boundary)
    neg_mask = da_los_values < 0
    if bool(neg_mask.any().values):
        # get max time (day of year) from the input timeseries (same approach as original)
        da_max_times = da["time"].dt.dayofyear.isel(time=-1)

        # negatives: eos - sos (will be negative)
        da_neg_values = da_eos_doy.where(neg_mask) - da_sos_doy.where(neg_mask)

        # replace negative values with max_time + negative_difference (same formula as original)
        da_los_values = xr.where(
            da_los_values >= 0, da_los_values, da_max_times + da_neg_values
        )

        # drop time dim if exists (keeps parity con versión original)
...
def get_roi(da_peak_values, da_peak_times, da_sos_values, da_sos_times):
    """
    Igual que el original, pero funciona si da_peak_times/da_sos_times son datetime64.
    Calcula ROI = (peak_value - sos_value) / (peak_time - sos_time) con el
    denominador en días (day-of-year), manteniendo la lógica simple del original.
    """
    ...
    # --- Normalizar tiempos a day-of-year si vienen como datetime64 ---
    # Si ya son numéricos (ej int day-of-year), quedan igual.
    try:
        # si son datetime64 -> .dt.dayofyear
        peak_times_doy = da_peak_times.dt.dayofyear
    except Exception:
        peak_times_doy = da_peak_times

    try:
        sos_times_doy = da_sos_times.dt.dayofyear
    except Exception:
        sos_times_doy = da_sos_times

    # get ratio between the difference in peak and sos values and times (in days)
    logger.info("> Calculating rate of increase (roi) values.")
    denom = peak_times_doy - sos_times_doy

    # evitar división por cero: reproducimos comportamiento simple del original,
    # pero prevenimos inf reemplazando ceros por NaN
    denom = xr.where(denom == 0, np.nan, denom)

    da_roi_values = (da_peak_values - da_sos_values) / denom
...
def get_rod(da_peak_values, da_peak_times, da_eos_values, da_eos_times):
    """
    Igual comportamiento que el original, pero acepta da_peak_times/da_eos_times como
    datetime64. Calcula ROD = abs((eos_value - peak_value) / (eos_time - peak_time))
    donde el denominador está en días (day-of-year). No aplica corrección por cruce de año.
    """
    ...
    try:
        eos_times_doy = da_eos_times.dt.dayofyear
    except Exception:
        eos_times_doy = da_eos_times

    # get abs ratio between the difference in peak and eos values and times
    logger.info("> Calculating rate of decrease (rod) values.")
    denom = eos_times_doy - peak_times_doy

    # evitar división por cero (mismo comportamiento simple que usamos en get_roi)
    denom = xr.where(denom == 0, np.nan, denom)

    da_rod_values = abs((da_eos_values - da_peak_values) / denom)
...
def get_lios(da, da_sos_times, da_eos_times):
    """
    Takes three xarray DataArrays containing veg values and sos/eos times (datetime64
    or day-of-year) to calculate the long integral of season (lios) per pixel using
    the trapezoidal rule. Behaviour matches the original but accepts datetime64.
    """
    ...
    # prepare boolean mask: soporta da_sos_times/da_eos_times como datetime64 o como day-of-year numérico
    logger.info("> Calculating long integral of season (lios) values.")

    # detect if times are datetime64 (por pixel)

    sos_is_dt = np.issubdtype(getattr(da_sos_times, "dtype", object), np.datetime64)
    eos_is_dt = np.issubdtype(getattr(da_eos_times, "dtype", object), np.datetime64)

    if sos_is_dt or eos_is_dt:
        # comparar con coordenada temporal completa (datetime64)
        # broadcasting: da['time'] (1D) se comparará por píxel con da_sos_times/da_eos_times (2D)
        mask = (da["time"] >= da_sos_times) & (da["time"] <= da_eos_times)
    else:
        # asumimos day-of-year numérico: usar .dt.dayofyear sobre da['time']
        mask = (da["time"].dt.dayofyear >= da_sos_times) & (
            da["time"].dt.dayofyear <= da_eos_times
        )

    # seleccionar valores dentro del periodo, fuera -> 0 (igual que original)
    da_lios_sel = da.where(mask, 0)

    # calcular lios con trapz a lo largo de 'time' (dx=1, igual que el original)
    ...
    # convertir tipo y renombrar (igual que el original)
    da_lios_values = da_lios_values.astype("float32").rename("lios_values")
...
def get_sios(da, da_sos_times, da_eos_times, da_base_values):
    """
    Igual comportamiento que el original, pero acepta da_sos_times/da_eos_times como
    datetime64 o como day-of-year numérico.
    """
    # notify user
    logger.info(
    ...
    logger.info("> Calculating short integral of season (sios) values.")

    # detectar si los tiempos vienen como datetime64 (por píxel) o como números (day-of-year)
    sos_is_dt = np.issubdtype(getattr(da_sos_times, "dtype", object), np.datetime64)
    eos_is_dt = np.issubdtype(getattr(da_eos_times, "dtype", object), np.datetime64)
    ...
    # (mismo criterio de comparación que arriba, aplicado sobre la variable con dimensión time)
    if sos_is_dt or eos_is_dt:
        mask_bse = (da_sios_bse_values["time"] >= da_sos_times) & (
            da_sios_bse_values["time"] <= da_eos_times
        )
    else:
        mask_bse = (da_sios_bse_values["time"].dt.dayofyear >= da_sos_times) & (
            da_sios_bse_values["time"].dt.dayofyear <= da_eos_times
        )

    da_sios_bse_sel = da_sios_bse_values.where(mask_bse, 0)
...
def get_liot(da):
    """
    Long integral of total (liot) using trapezoidal rule.
    If da.time is datetime64, uses the mean time-step in DAYS as dx; otherwise uses dx=1.
    Returns same shape as original (no time dim).
    """
    ...
    # determine dx: if datetime64 -> compute mean sampling interval in days
    dx = 1.0
    try:
        t = da["time"].values
        if np.issubdtype(t.dtype, np.datetime64) and t.shape[0] > 1:
            # time values as int ns since epoch
            t_ns = t.astype("datetime64[ns]").astype("int64")
            diffs_ns = np.diff(t_ns)  # nanoseconds
            mean_ns = np.mean(diffs_ns)
            # convert ns -> days
            dx = float(mean_ns) / 1e9 / 86400.0
            # warn if irregular spacing
            if not np.allclose(diffs_ns, mean_ns):
                logger.info(
                    "> Warning: time spacing is irregular. Using mean dx = {:.6f} days.".format(
                        dx
                    )
                )
            else:
                logger.info(
                    "> Detected datetime time coordinate. Using dx = {:.6f} days.".format(
                        dx
                    )
                )
    except Exception:
    ...
    logger.info("> Calculating long integral of total (liot) values.")
...
def get_siot(da, da_base_values):
    """
    Short integral of total (siot) using trapezoidal rule.
    Igual al original, pero acepta coordenadas de tiempo datetime64 y ajusta dx en días.
    """
    ...
    # --- calcular dx en días si 'time' es datetime64 ---
    dx = 1.0
    try:
        t = da["time"].values
        if np.issubdtype(t.dtype, np.datetime64) and t.shape[0] > 1:
            t_ns = t.astype("datetime64[ns]").astype("int64")
            diffs_ns = np.diff(t_ns)
            mean_ns = np.mean(diffs_ns)
            dx = float(mean_ns) / 1e9 / 86400.0  # ns -> days
            if not np.allclose(diffs_ns, mean_ns):
                logger.info(
                    f"> Warning: irregular time spacing. Using mean dx = {dx:.6f} days."
                )
            else:
                logger.info(f"> Detected datetime time coordinate. Using dx = {dx:.6f} days.")
    except Exception:
        dx = 1.0

    # calculate siot using trapz
    logger.info("> Calculating short integral of total (siot) values.")
    ...
    # project base values to time dimension (rectangle under curve)
    da_siot_bse_values = da_base_values.combine_first(da)

    # calculate trapz of base
...
def calc_phenometrics(
    ...
    # notify user
    logger.info("Initialising calculation of phenometrics.\n")

    # check if dask - not yet supported
    if dask is not None and dask.is_dask_collection(da):
    ...
    # takes peak and base arrays
    if peak_metric == "pos" and base_metric == "bse":
        da_aos_values = get_aos(
            da_peak_values=da_pos_values, da_base_values=da_bse_values
        )
    elif peak_metric == "pos" and base_metric == "vos":
        da_aos_values = get_aos(
            da_peak_values=da_pos_values, da_base_values=da_vos_values
        )
    elif peak_metric == "mos" and base_metric == "bse":
        da_aos_values = get_aos(
            da_peak_values=da_mos_values, da_base_values=da_bse_values
        )
    elif peak_metric == "mos" and base_metric == "vos":
        da_aos_values = get_aos(
            da_peak_values=da_mos_values, da_base_values=da_vos_values
        )
    ...
    da_lios_values = get_lios(
        da=da, da_sos_times=da_sos_times, da_eos_times=da_eos_times
    )
    ...
    # ver esta parte de abajo!
    da_list = [da.drop_vars("time", errors="ignore") for da in da_list]
    ...
    # notify user
    logger.info("Phenometrics calculated successfully!")
